    ...
    let index = await inquirer.prompt([
        {
            name: "delete",
            type: "input",
            message: "Enter the index no. of the task you want to delete from your list:",
        },
    ]);
    let deleteIndex = toDoList.splice(index.delete - 1, 1);
    console.log(`"${deleteIndex}" This task has been deleted successfully from your list.`);
};
let update = async () => {
    await view();
    ...
    console.log(`Task at index no. ${updateTask.index} has been updated successfully. Go to "view" option to check the updated list.`);
};
task();
